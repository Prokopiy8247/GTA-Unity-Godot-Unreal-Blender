# GPT-6 Astra — Unity + Blender MCP Realistic GTA-Style Open-World Benchmark

## Mission

You are the sole lead developer and technical artist of the **existing Unity project in the currently opened Codex Desktop folder**.
...
Work directly in the current Unity project. **Do not create another nested Unity project.**

A Blender file for this Unity project already exists:
...
- use **only** the MCP server `blender_unity`;
- use the Blender instance on port **9876**;
- use `UnityGTA.blend` as the canonical Blender working file for this project;
- do **not** use `blender_godot`;
- do **not** use `blender_unreal`;
- do not modify Blender files belonging to the other engine projects.

Before making major Blender changes, verify that the connected Blender scene is the intended Unity GTA asset scene.

Keep `UnityGTA.blend` saved regularly.

You may create automatic backups or split-out source `.blend` files if genuinely necessary for stability, but `UnityGTA.blend` remains the master Blender scene for the Unity benchmark.
...
2. model or modify the asset through `blender_unity`;
...
8. import it into Unity;
9. configure materials, colliders, physics and gameplay components;
10. test it in the actual game.

A Blender asset is **not finished merely because a file exists**. It must work and look reasonable inside Unity.

---

# 2. No external asset shortcuts

This benchmark is about **GPT-6 Astra + Unity + Blender MCP**.
...
Built-in Unity functionality and built-in packages are allowed.
...
1. inspect the entire Unity project;
2. detect the exact Unity version;
3. identify the current render pipeline;
4. identify installed packages;
5. inspect the existing `UnityGTA.blend`;
6. confirm access to `blender_unity`;
...
For this realism target, **HDRP is preferred** if:
- the project is sufficiently clean to migrate safely;
- the installed Unity version supports it;
- migration does not jeopardize project stability.

If the project is already HDRP, keep and use it.

If the project is URP or Built-in:
- evaluate migration early;
- migrate to HDRP only if it can be done reliably;
- otherwise maximize realism using the existing pipeline and document the limitation.

Do not destroy a working project merely to force HDRP.

If HDRP is used, take advantage of appropriate features such as:
- physically based lighting;
- volumetric fog;
- decals;
- reflection probes / screen-space reflections where appropriate;
- contact shadows;
- high-quality post processing;
- terrain/vegetation detail;
- local lights;
- emissive materials;
- physically based sky/atmosphere where practical.
...
### Roads
Use Unity tooling/code for road layout where more efficient, but create necessary visual assets through Blender:
- curbs;
- barriers;
- signs;
- lamp posts;
...
Use Unity Humanoid/Mecanim-compatible conventions where practical.
...
Use Unity Rigidbody/WheelCollider or another reliable project-owned physics solution.
...
Major vehicles must be authored through `blender_unity`.
...
Inside `UnityGTA.blend`, organize major assets into clear collections such as:
...
Adapt this to the actual project as needed.

---

# 41. Blender export pipeline

Create a reproducible Blender → Unity pipeline.
...
→ save source
→ export FBX/GLB
→ Assets/GTA/Generated/Models/...
→ Unity import
→ material assignment
→ prefab generation/configuration
→ gameplay integration
```

For rigged characters and animated vehicles/parts, FBX is generally preferred.

For simple static assets, use FBX or GLB based on whichever is more reliable in the actual Unity version.

Do not rely on manually exporting every asset through Blender's GUI.

Use Blender MCP, Blender Python invoked through MCP when appropriate, or reproducible export automation.
...
Create appropriate LOD strategy for:
- vehicles;
- buildings;
- trees;
- street props;
...
Use Unity LODGroup where appropriate.
...
Use Unity's available VFX/particle system appropriate to the current pipeline.
...
Avoid:
- one MonoBehaviour per tiny prop where unnecessary;
...
Use Unity batch mode/editor automation when helpful.

Do not wait until the very end to discover that the project does not compile.
...
Then validate the exported result inside Unity.

---

# 56. Final Unity validation

Before declaring completion:

1. project opens cleanly;
2. no critical compiler errors;
3. default scene loads;
4. player can move;
5. at least several districts are accessible;
6. traffic works;
7. pedestrians work;
8. vehicles work;
...
19. admin menu works;
20. Blender-generated assets display correctly;
21. no major missing materials;
22. save/load works at least at basic level;
23. performance is acceptable for recording.
...
These metrics will be shown in the YouTube comparison between Unity, Godot and Unreal Engine, so collect them carefully and **do not fabricate precision**.
...
If Blender MCP, local tools, Unity, or other local tool calls do not have a separately metered API fee, do not invent one.
...
- other standard Unity transient data.
...
2. fix critical compile/runtime errors;
3. inspect `FEATURE_MATRIX.md`;
4. update `ASTRA_FINAL_REPORT.md`;
5. save `UnityGTA.blend`;
6. ensure exported Blender assets exist in the Unity project;
...
- verify `blender_unity`;
- verify `UnityGTA.blend`;
- record start time;
...
**`blender_unity` on port 9876 and `UnityGTA.blend` are the mandatory Blender asset pipeline.**
