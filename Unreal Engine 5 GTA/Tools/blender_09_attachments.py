...
parts=[rod('SuppressorTube',(0,0,0),(.22,0,0),.019,black,32),rod('Mount',(-.02,0,0),(.025,0,0),.023,metal,32)]
for i in range(7):parts.append(rod('MachinedRing',(.04+i*.022,0,0),(.046+i*.022,0,0),.021,black,32))
parts.append(rod('DarkBore',(.221,0,0),(.223,0,0),.008,dark,20))
finish('SM_Suppressor',parts,'Weapons').location=(24,170,0)
parts=[box('RailClamp',(0,0,0),(.045,.039,.018),metal,.005),rod('Foregrip',(0,0,-.01),(.017,0,-.115),.020,dark,20,.014)]
for i in range(6):parts.append(rod('GripRing',(.003+i*.002,0,-.025-i*.012),(.003+i*.002,0,-.029-i*.012),.0205,black,20))
finish('SM_Foregrip',parts,'Weapons').location=(25,170,0)
parts=[box('MagazineBody',(0,0,-.06),(.05,.032,.12),black,.008),box('BasePlate',(0,0,-.126),(.064,.042,.016),metal,.004)]
for s in [-1,1]:
 for i in range(4):parts.append(box('StampedGroove',(.009,s*.0168,-.04-i*.02),(.025,.002,.004),dark,.001))
finish('SM_ExtendedMagazine',parts,'Weapons').location=(26,170,0)
bpy.ops.wm.save_as_mainfile(filepath=ROOT+'UnrealGTA.blend')
print('Three functional firearm attachments exported.')
